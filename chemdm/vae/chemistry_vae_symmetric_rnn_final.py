...
def save_models(encoder, decoder, epoch, settings, alphabet):
    ...
    '''Arguments:
                    ...
                    alphabet: the alphabet generated by the function get_selfie_and_smiles_encodings_for_dataset'''
...
def save_models_epoch_loss(epoch, loss, recon_loss, val_loss, kld_loss, settings):
    ...
    '''Arguments:
                    ...
                    recon_loss: the training reconstruction loss
                    val_loss: the total validation loss
                    kld_loss: the training kld loss
                    settings: settings defined by the .yml file'''
...
class VAEEncoder(nn.Module):
    ...
    def __init__(self, in_dimension, gru_stack_size, gru_neurons_num,
        ...
        '''Arguments:
                        in_dimension: the number of elements in each one hot array
                        gru_stack_size: the number of gru_stacks
                        gru_neurons_num: the number of neurons
                        latent_dimension: the size of the latent space'''
    ...
    def reparameterize(mu, log_var):
        ...
        '''Arguments:
                        mu: the mean latent vector
                        log_var: the natural logarithm of the variance tensor of each latent vector'''
        
        '''Outputs:
                        z: a latent vector modified by some distribution defined by the standard deviation'''
    ...
    def forward(self, x, hidden=None):
        ...
        '''Arguments:
                        x: The feed forward object, i.e., the transformed one hot vector'''
...
class VAEDecoder(nn.Module):
    ...
    def __init__(self, latent_dimension, gru_stack_size, gru_neurons_num,
        ...
        '''Arguments:
                        latent_dimension: the size of the latent space
                        gru_stack_size: the number of gru layers
                        gru_neurons_num: the number of neurons in each layer
                        out_dimension: the length of the selfies_alphabet'''
    ...
    def forward(self, z, hidden):
        ...
        '''Arguments:
                        z: the latent vector
                        hidden: the hidden layer/s'''
        
        '''Outputs:
                        decoded: the decoded output, should be a one hot vector
                        hidden: the output hidden layer'''
...
def train_model(vae_encoder, vae_decoder, data_train, data_valid ,num_epochs, batch_size,
    ...
    '''Arguments:
                    vae_encoder: the encoder object
                    vae_decoder: the decoder object
                    data_train: the training one hot vector set of the molecules
                    data_valid: the validation one hot vector set of the molecules
                    num_epochs: the desired number of epochs
                    batch_size: the batch size
                    lr_enc: the learning rate for the encoder training
                    lr_dec: the learning rate for the decoder training
                    KLD_alpha: the weight you apply to the KLD term
                    alphabet: the alphabet generated by the function get_selfie_and_smiles_encodings_for_dataset
                    settings: settings defined by the yml file'''
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 50 == 0:
            save_models(vae_encoder, vae_decoder, epoch, settings, alphabet)
        ...
        save_models_epoch_loss(epoch, loss.item(), recon_loss, val_loss.item(), kld_loss, settings)
...
def compute_elbo(x, x_hat, mus, log_vars, KLD_alpha):
    ...
    '''Arguments:
                    x: the batch/ the one hot vector object defined by the batch size
                    x_hat: the one hot vector object after it has been fed through the encoder and decoder
                    mus: the mean latent vector object
                    log_vars: the natural logarithm of the variance tensor of each latent vector
                    KLD_alpha: the weight you apply to the KLD term'''
    
    '''Outputs:
                    total_loss: the total training loss
                    recon_loss: the reconstruciton training loss
                    KLD loss: the KLD training loss'''
...
def validation_loss(x, x_hat, mus):
    ...
    '''Outputs:
                    recon_loss: the validation reconstruct loss'''
...
def get_selfie_and_smiles_encodings_for_dataset(file_path):
    ...
    '''Arguments:
                    file_path: .csv file with molecules. Column name containing the smiles must be 'smiles' '''
    
    '''Outputs:
                    selfies_list: the SELFIES encoding of the SMILES molecules provided
                    selfies_alphabet: the alphabet of the SELFIES encoding
                    largest_selfies_len: the longest SELFIES encoding length
                    smiles_list: a list of the SMILES encodings
                    smiles_alphabet: the alphabet of the SMILES encoding
                    largest_smiles_len: the longest SMILES encoding length'''
...
def remove_unrecognized_symbols(smiles_list):
    ...
    '''Arguments:
                    smiles_list: the list of SMILES encodings'''
    
    '''Outputs:
                    cleaned_smiles: the cleaned SMILES encodings list'''
...
def data_init(settings, device):
    ...
    '''Outputs:
                    data_train: the training one hot vector set
                    data_valid: the validation one hot vector set
                    len_max_molec: the maximum length of the molecule encodings
                    len_alphabet: the length of the alphabet used
                    len_max_mol_one_hot: the length of the maximum one hot representation
                    encoding_alphabet: the alphabet used'''
