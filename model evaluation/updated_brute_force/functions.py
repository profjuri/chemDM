...
def lpoints_to_onehots(zs, selfies_alphabet, vae_decoder, len_max_molec):

    data_list = []
    zs = zs.squeeze().to('cpu')
    ...
    for batch_iteration in range(num_batches_train):
        ...
        sequences = seq_tensor.squeeze().t().to(torch.int64)
        if sequences.dim() < 2:
            sequences = sequences.unsqueeze(0)
        
        data = torch.nn.functional.one_hot(sequences, num_classes = len(selfies_alphabet)).to(torch.float32)
        data_list.append(data)


    one_hot_tensor = torch.cat(data_list)
