...
def save_models(encoder, decoder, epoch, lr_new_enc, KLD, settings, alphabet):
    out_dir = '../selfies_saved_models_qm9/' + 'lr_' + str(lr_new_enc)  + '_KLD_' + str(KLD) + '/{}'.format(epoch)  
    _make_dir(out_dir)
...
def save_models_epoch_loss(epoch, loss, lr_new_enc, KLD):
    out_dir = '../selfies_saved_models_qm9/' + 'lr_' + str(lr_new_enc)  + '_KLD_' + str(KLD) + '/0'
    log_folder = out_dir  # Replace with the desired folder path
...
class VAEEncoder(nn.Module):
    ...
    def __init__(self, in_dimension, layer_1d, layer_2d, layer_3d, layer_4d, layer_5d,
                 latent_dimension):
        """
        ...
        self.encode_nn = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Linear(layer_3d, layer_4d),
            nn.BatchNorm1d(layer_4d),
            nn.LeakyReLU(),
            nn.Linear(layer_4d, layer_5d),
            nn.BatchNorm1d(layer_5d),
            nn.LeakyReLU()
        )

        # Latent space mean
        self.encode_mu = nn.Linear(layer_5d, latent_dimension)

        # Latent space variance
        self.encode_log_var = nn.Linear(layer_5d, latent_dimension)
...
def train_model(vae_encoder, vae_decoder,
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer_encoder, step_size=25, gamma=0.9)
    scheduler2 = optim.lr_scheduler.StepLR(optimizer_decoder, step_size=25, gamma=0.9)
...
def main():
    ...
    if os.path.exists("selfies_settings.yml"):
        settings = yaml.safe_load(open("selfies_settings.yml", "r"))
    else:
