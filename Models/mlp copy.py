...
def train_model(encoding_list, encoding_alphabet, len_max_molec, vae_encoder, properties_tensor, optimizer, model, loss_function, scheduler, settings, epoch_full):
    ...
    for epoch in epoch_full:
        ...
        int_model_save(epoch, settings, optimizer, model, scheduler)

def reload_run(settings):

    '''Function to reload previously saved runs'''
    ...
    '''Outputs:
                    ...
                    optimizer: the optimizer used to modify the weights after a back propagation (Pytorch torch.optim object)
                    scheduler: function used to modify the learning rate (torch.optim.lr_scheduler object)
                    fin_epoch: the epoch in which the preloaded models were saved on (int)'''
    ...
    pre_load_dir = settings['settings']['output_folder'] + '/Pre_load/'

    encoding_df = pd.read_csv(pre_load_dir+'/encoding_df.csv')
    encoding_list = list(encoding_df['smiles'])
    properties_tensor = torch.tensor(encoding_df['prop_list'].tolist())
    ...
    load_settings = yaml.safe_load(open(pre_load_dir+'/settings.yml', "r"))
    learning_rate = load_settings['hyperparameters']['lr']
    learning_rate_factor = load_settings['hyperparameters']['learning_rate_factor']
    learning_rate_patience = load_settings['hyperparameters']['learning_rate_patience']
    dict_load = torch.load(pre_load_dir + '/state_dicts.pt')


    model = PropertyRegressionModel(load_settings).to(device)
    ...
    optimizer.load_state_dict(dict_load['optimizer_state_dict'])

    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=learning_rate_factor, patience=learning_rate_patience, verbose=True)
    scheduler.load_state_dict(dict_load['scheduler_state_dict'])

    
    fin_epoch = load_settings['additional']['fin_epoch']
    len_alphabet = len(encoding_alphabet)

    return largest_molecule_len, len_alphabet, len_max_molec_onehot, encoding_alphabet, encoding_list, vae_encoder, properties_tensor, model, optimizer, scheduler, fin_epoch


def int_model_save(epoch, settings, optimizer, model, scheduler):

    '''Intermediate model saving function'''
    ...
    pre_load_dir = settings['settings']['output_folder'] + '/Pre_load/'
    settings['additional'] = {'fin_epoch': epoch}

    torch.save({
            'optimizer_state_dict': optimizer.state_dict(),
            'model_state_dict': model.state_dict(),
            'scheduler_state_dict':scheduler.state_dict()
            }, pre_load_dir + '/state_dicts.pt')

    data = {**settings}
    with open(pre_load_dir+'/settings.yml', 'w') as file:
        yaml.dump(data, file)
    

def dump_enc(settings, encoding_list, properties_tensor):

    '''Function to dump the encoding list into a csv file so that it can be easily reloaded'''
    ...
    prop_list = properties_tensor.squeeze().tolist()
    df = pd.DataFrame({'smiles': encoding_list, 'prop_list': prop_list})
    output_folder = settings['settings']['output_folder']
    os.makedirs(output_folder+'Pre_load')
    
    df.to_csv(output_folder+'/Pre_load/encoding_df.csv', index =False)
...
def main():
    ...
    if os.path.exists(pre_load_dir):

        len_max_molec, _, _, encoding_alphabet, encoding_list, vae_encoder, properties_tensor, model, optimizer, scheduler, fin_epoch = reload_run(settings)
        epoch_list = epoch_list[(fin_epoch+1):]

        print('Load completed')
    else:

        len_max_molec, _, _, encoding_alphabet, encoding_list, vae_encoder, properties_tensor = data_init(settings)
        dump_enc(settings, encoding_list, properties_tensor)
