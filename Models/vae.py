...
def save_models(encoder, decoder, epoch, settings, encoding_alphabet):
...
def train_model(vae_encoder, vae_decoder, num_epochs, batch_size,
                ...
                largest_molecule_len, encoding_alphabet, settings, encoding_list):
    ...
    for epoch in range(num_epochs):
        ...
        print('Time for epoch:', time.time() - time_0)
        time_0 = time.time()
        ...
        for chunk_iteration in range(train_chunk):
            ...
            for cluster_iteration in range(num_clusters):
                ...
                for batch_iteration in range(num_batches_train):  # batch iterator
                    ...
                    decode_times.append(time.time() - decode_time)
        ...
        print('TOTAL TIME DECODING:', sum(decode_times))
        ...
        if epoch % 10 == 0:
            save_models(vae_encoder, vae_decoder, epoch, settings, encoding_alphabet)
...
def main():
    ...
    largest_molecule_len, len_alphabet, len_max_mol_one_hot, encoding_alphabet, encoding_list = data_init(settings)
