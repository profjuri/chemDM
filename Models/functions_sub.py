...
def sequences_to_smiles(scattered_seq_tensor, encoding_alphabet):

    '''Takes sequence output from VAE Decoder and converts them to SMILES'''

    '''Arguments:
                    scattered_seq_tensor: pytorch sequence tensor representing the SELFIES output from the decoder. Sequence numbers correspond to alphabet characters (Pytorch float.32 tensor)
                    encoding_alphabet: the alphabet of the SELFIES encoding (list)'''  
    ...
    encoding_alphabet_np = np.array(encoding_alphabet)
    continuous_strings = encoding_alphabet_np[scattered_seq_tensor]
...
def sanitise_df(path):

    '''This function is used to fully sanitise input ideal_mol dfs'''

    '''Arguments:
                    path: path to the ideal_mols DF, it will either be the path to 1 or 2 (str)'''
    ...
    if 'SAS' not in df.columns:
        print(f'Beginning ID validation')
        ID_smiles = df['canon_smiles'].tolist()
        valid_list  = []
        for x in range(len(ID_smiles)):
            try:
                y = Chem.MolFromSmiles(ID_smiles[x])
                if y != None:
                    valid_list.append(x)
            except Exception as e:
                pass
        
        df = df.iloc[valid_list]
        ID_smiles = df['canon_smiles'].tolist()

        print(f'Beginning ID SA score calculation')

        sascores_ID = [sascorer.calculateScore(Chem.MolFromSmiles(x)) for x in ID_smiles]
        df['SAS'] = sascores_ID
