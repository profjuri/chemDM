...
def mu_gen_step(zs, threshold_indices, selfies_alphabet, vae_decoder, vae_encoder):
    ...
    del zs
    lpoints_decoded = sequences_to_lpoints(scattered_seq_tensor, selfies_alphabet, vae_encoder).to(device)
    del scattered_seq_tensor
...
def sub_df_prep(settings, df_sub):
    ...
    '''Arguments:
                        settings: dict of settings used (dict)
                        df_sub: pandas dataset containing a smaller version of the dataframe from sanitised_bf_results.csv (Pandas DataFrame object)'''
...
def load_mlpB(settings):
    ...
    '''Function to load the network B MLP'''
...
def pd_sorting(settings, predictions, file_path, start_idx, stop_idx, B_OS_THRESHOLD, B_dE_THRESHOLD):
    ...
    '''Arguments:
                        settings: dict of settings used (dict)
                        predictions: predictions from network B MLP, should contain dE1,2 and OS1,2. ordered: [dE1, OS1, dE2, OS2] (Pytorch float.32 tensor)
                        file_path: save path for the brute force code (str)
                        start_idx: starting idx for the pandas dataframes (int)
                        stop_idx stop idx for the pandas dataframes (int)
                        B_OS_THRESHOLD: oscillator strength threshold for network B (float)
                        B_dE_THRESHOLD: transition energy threshold for network B (float)'''


    predictions[:, 1::2] = (10**(predictions[:, 1::2])) / predictions[:, ::2]

    trans1 = [predictions[x, 0].item() for x in range(predictions.shape[0])]
    osc1 = [predictions[x, 1].item() for x in range(predictions.shape[0])]
