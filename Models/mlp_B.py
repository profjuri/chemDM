...
import vae
from functions_sub import stats, selfies_to_zs, gen_properties_tensor, selfies_to_one_hot, selfies_to_lpoints_slow, get_free_ram, get_free_memory, gen_fingerprints
...
class PropertyRegressionModel(nn.Module):
    def __init__(self, settings):
        ...
        self.linear3 = nn.Linear(2048, input_dim)
...
def data_init(settings):
    ...
    return largest_molecule_len, len_alphabet, len_max_molec_onehot, encoding_alphabet, encoding_list, vae_encoder, properties_tensor, smiles_list
...
def main():
    ...
    if os.path.exists("mlpB_settings.yml"):
        settings = yaml.safe_load(open("mlpB_settings.yml", "r"))
    else:
    ...
    len_max_molec, _, _, encoding_alphabet, encoding_list, vae_encoder, properties_tensor, smiles_list = data_init(settings)
