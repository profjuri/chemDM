...
from functions import selfies_to_one_hot, _make_dir, get_free_memory, get_selfie_and_smiles_encodings_for_dataset, remove_unrecognized_symbols
...
class VAEDecoder(nn.Module):
    ...
    def forward(self, z, hidden):
        ...
        '''Arguments:
                        ...
                        hidden: the hidden layer/s (Pytorch float.32 tensor)'''
        
        '''Outputs:
                        decoded: the decoded output, should be a one hot vector (Pytorch float.32 tensor)
                        hidden: the output hidden layer (Pytorch float.32 tensor)'''
...
def train_model(vae_encoder, vae_decoder, num_epochs, batch_size,
                ...
                sample_len, alphabet, settings, encoding_list):
    ...
    '''Arguments:
                    ...
                    sample_len: the maximum molecule length in terms of SELFIES characters (int)
                    alphabet: the alphabet generated by the function get_selfie_and_smiles_encodings_for_dataset (list)
                    settings: settings defined by the yml file (dict)
    ...
    total_mem_req = (4*(len(encoding_list)*sample_len*len(alphabet)))
    free_memory = get_free_memory(device)
    memory_ratio = total_mem_req/free_memory + 1

    num_clusters = int(memory_ratio)
    print('Number of clusters required:', num_clusters)
    ...
    for epoch in range(num_epochs):
        ...
        for chunk_iteration in range(train_chunk):
            ...
            data_train = selfies_to_one_hot(sub_encoding, alphabet, sample_len)
        ...
        with torch.no_grad():
            for chunk_iteration in range(valid_chunk):
                ...
                sub_encoding = encoding_list[start_idx: stop_idx]
                data_valid = selfies_to_one_hot(sub_encoding, alphabet, sample_len)
...
def data_init(settings):
    ...
    return largest_molecule_len, len(encoding_alphabet), largest_molecule_len*len(encoding_alphabet), encoding_alphabet, encoding_list
...
def main():
    ...
    len_max_molec, len_alphabet, len_max_mol_one_hot, encoding_alphabet, encoding_list = data_init(settings, device)
