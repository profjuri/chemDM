...
def find_links(df, threshold):
    ...
    if len(fps) < fp_batch_size:
        fp_num_batches = 1
    else:
        fp_num_batches = int(len(fps) / fp_batch_size)
        if fp_num_batches * fp_batch_size < len(fps):
            fp_num_batches +=1
    ...
    for fp_idx in range(fp_num_batches):
        ...
        sub_fps = fps[fp_start_idx:fp_stop_idx]
