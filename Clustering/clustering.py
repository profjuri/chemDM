...
def data_init(interested_path):
    ...
    for x in range(len(smiles_list)):
        ...
        if x % 100000 ==0:
            print(f'Validation step: {x}/{len(smiles_list)}, time taken: {time.time()-t_}, validation list length: {len(valid_list)}')
            t_ = time.time()

        sm = smiles_list[x]
        try:
            Chem.MolToSmiles(MurckoScaffold.GetScaffoldForMol(Chem.MolFromSmiles(sm)))
            valid_list.append(x)
        except Exception as e:
            pass

    df_new = df.iloc[valid_list].copy()
