...
def save_models(encoder, decoder, epoch, lr_new_enc, KLD, settings, alphabet):
    ...
    out_dir = str(save_path) + 'stack_size' + str(gru_stack_size)  + 'neurons_num' + str(gru_neurons_num) + '/{}'.format(epoch)  
    _make_dir(out_dir)
...
def save_models_epoch_loss(epoch, loss, lr_new_enc, KLD, settings):
    ...
    out_dir = str(save_path) + 'stack_size' + str(gru_stack_size)  + 'neurons_num' + str(gru_neurons_num) + '/0'
    log_folder = out_dir  # Replace with the desired folder path
...
def train_model(vae_encoder, vae_decoder,
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer_encoder, step_size=5, gamma=0.75)
    scheduler2 = optim.lr_scheduler.StepLR(optimizer_decoder, step_size=5, gamma=0.75)
    ...
    for epoch in range(num_epochs):
        ...
        save_models_epoch_loss(epoch, avg_loss, lr_enc, KLD_alpha, settings)
...
def main():
 ...
 #         f"largest molecule is {len_max_molec} letters.")
    ...
    for i in range(num_tests):
        ...
        #encoder_parameter['gru_neurons_num'] = 2100
        #encoder_parameter['gru_stack_size'] = 3#random.randint(2)


        decoder_parameter = settings['decoder']
        #decoder_parameter['gru_neurons_num'] = encoder_parameter['gru_neurons_num']
        #decoder_parameter['gru_stack_size'] = encoder_parameter['gru_stack_size']
